...
namespace MMBot.Slack
{
	...
	public class SlackAdapter : Adapter
	{
		...
		{
			if (this.regexUser.IsMatch(message) != true)
			{
				return message;
			}

			var userIdsInMesage = this.regexUser.Matches(message).Cast<Match>().Join(_users, match => match.Groups[1].Value, user => user.Id, (match, user) => user).Distinct();

			foreach (var user in userIdsInMesage)
			{
				message = Regex.Replace(message, string.Format(@"<@{0}\|?(?<userName>\w*)>", user.Id), string.Format("@{0}", user.Name));
			}
